      ...
      <header className="border-b-2 border-border bg-card sticky top-0 z-50">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-headline tracking-tighter">SOCRATIC </h1>
            <nav className="flex gap-1">
              <a href="/" className="px-4 py-2 text-sm font-medium border-b-2 border-accent transition-colors">
                Home
              </a>
              <a href="/about" className="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground border-b-2 border-transparent hover:border-border transition-colors">
                About
              </a>
            </nav>
          </div>
      ...
      <section className="py-12 px-4">
        <div className="container mx-auto max-w-6xl">
          ...
          {isLoading ? <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {topics?.map(topic => <TopicCard key={topic.id} title={topic.title} description={topic.description} onClick={() => handleTopicClick(topic.id)} />)}
            </div>}
        </div>
      </section>
    </div>;
